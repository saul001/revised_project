...
- Configurable-horizon forecast for new/latest data, with an illustrative
  uncertainty band, and CSV download
...
st.set_page_config(
    page_title="NEPSE Manufacturing Stock Prediction",
    layout="wide",
)

# ---------------------------------------------------------------------------
# Minimal styling for a cleaner, more consistent look
# ---------------------------------------------------------------------------
st.markdown("""
<style>
    ...
    div[data-testid="stMetricLabel"] {font-weight: 500; opacity: 0.75;}
...
if data_source == "Upload new CSV":
    ...
    if uploaded is not None:
        try:
            raw_df = _load_raw_csv(uploaded)
        except ValueError as e:
            st.sidebar.error(f"Could not read this CSV: {e}")
        except Exception as e:
            st.sidebar.error(f"Could not read this CSV — unexpected error: {e}")
else:
    if os.path.exists(config.DEFAULT_CSV):
        try:
            raw_df = _load_raw_csv(config.DEFAULT_CSV)
        except ValueError as e:
            st.sidebar.error(f"The bundled dataset could not be read: {e}")
    else:
        st.sidebar.error("No bundled dataset found. Place your CSV at "
                          "data/nepse_manufacturing.csv.")
...
symbol = st.sidebar.selectbox("Company", available_companies,
...
forecast_days = st.sidebar.slider("Forecast horizon (trading days)", 1, 14,
                                   config.FORECAST_HORIZON_DAYS)

st.sidebar.divider()
st.sidebar.caption(
    "NEPSE trades Sunday-Thursday; forecasts automatically skip Friday and Saturday."
)
...
if not models_ready:
    ...
    if st.button(f"Quick-train LSTM and GRU for {symbol}", type="primary"):
        from src import train as train_mod

        with st.spinner(f"Training LSTM and GRU for {symbol} — this can take a few minutes..."):
...
tab_overview, tab_perf, tab_confusion, tab_pred, tab_forecast = st.tabs(
    ["Overview", "Model Performance", "Confusion Matrix", "Actual vs Predicted", "Forecast"]
)
...
with tab_overview:
    ...
    c4.metric("Date Range", f"{sub['date'].min().date()} to {sub['date'].max().date()}")
    ...
    if metrics_summary is not None:
        ...
        if not naive_row.empty:
            ...
            st.caption(
                ...
                f"{naive_rmse:.3f}. 'Accuracy (%)' = 100 minus MAPE is inflated on price "
                f"levels, so a model only shows genuine predictive skill if its RMSE is "
        ...
        if not row.empty:
            ...
            if not naive_row.empty:
                ...
                for col, (_, r) in zip(status_cols, row.iterrows()):
                    with col:
                        if r["RMSE"] < naive_rmse:
                            st.success(f"{r['Model']}: beats the naive baseline (RMSE {r['RMSE']:.3f} "
                                       f"vs. {naive_rmse:.3f}).")
                        else:
                            st.warning(f"{r['Model']}: does not beat the naive baseline (RMSE "
                                       f"{r['RMSE']:.3f} vs. {naive_rmse:.3f}).")
                        if r.get("Echoes_Yesterday", False):
                            st.warning(f"{r['Model']}: lag-echo pattern detected — predictions "
                                       f"align more closely with yesterday's price than today's, "
                                       f"suggesting the model may largely be copying the last "
                                       f"observed close.")
...
with tab_pred:
    ...
    if fig.data:
        ...
        st.markdown("**Prediction residuals (Actual minus Predicted)**")
        resid_fig = go.Figure()
...
# ---------------------------------------------------------------------------
# Tab 5: N-day forecast on new data
# ---------------------------------------------------------------------------
with tab_forecast:
    st.subheader(f"{symbol} — {forecast_days}-Day Ahead Forecast")
    st.caption(
        "Recursively forecasts future NEPSE trading days (Sunday-Thursday) using the "
        "most recent available data. Each step recomputes technical indicators from "
        "the growing history and feeds the model's own prediction back in — a "
        "standard approach for multi-step time-series forecasting. The shaded band "
        "is an illustrative uncertainty range based on recent volatility, not a "
...
st.caption(
    ...
    "LSTM and GRU. Predictions are for academic purposes only and are not financial advice."
)
